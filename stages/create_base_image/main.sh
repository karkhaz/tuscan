...
reflector --sort rate --threads 2 -c "United States" --fastest 30 \
    --age 24 --protocol http --save /etc/pacman.d/mirrorlist
...
pacman -S base base-devel abs python python-requests parallel \
  ...
  binutils bison cmake ninja make libunistring patch
