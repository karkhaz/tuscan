...
def substitute_vars(data_structure, args):
    ...
    if isinstance(data_structure, bool):
        ...
        ret = sub("__TOUCH_DIR__", args.touch_dir, ret)
        if args.verbose:
            ret = sub("__VERBOSE__", "-v", ret)
        else:
            ret = sub("__VERBOSE__", "", ret)
...
class Stages(object):
    ...
    def write_run_recipies(self):
        for sta in self.stages:
            ...
            if (self.args.top_level == sta["name"]
            or "top_level" in sta["run"]):
    ...
    def normalise_containers(self):
        ...
        for sta in self.stages:
            ...
            if not "local_mounts" in sta["run"]["dependencies"]:
                sta["run"]["dependencies"]["local_mounts"] = []
            ...
            if not "rm_container" in sta["run"]:
                sta["run"]["rm_container"] = True
            ...
            for script in sta["run"]["dependencies"]["stages"]:
                ...
                if not tmp:
                    raise RuntimeError(
                        ...
                        "'" + script + "' having run, but that stage"
                        " doesn't exist.")
