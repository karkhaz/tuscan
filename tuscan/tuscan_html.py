...
def create_summary_pages(summary, ret, parent_name, builds, toolchains, jinja):
    ...
    if "name" in summary:
        ...
        _order = [basename(build["build_name"]) for build in order]
        order = []
        for e in _order:
            if e not in order:
                order.append(e)
...
def write_summary_pages(dst_dir, toolchains, builds, jinja):
    ...
    summary_out_dir = join(dst_dir, "summaries")
    if not isdir(summary_out_dir):
        makedirs(summary_out_dir)
    for f in listdir(summary_out_dir):
        unlink(join(summary_out_dir, f))
    ...
    for s in summaries["pages"]:
        ...
        name = s["name"]
        if s["name"] == "tuscan-all-builds-all":
            name = "index"
        with open(join(summary_out_dir, "%s.html" % name), "w") as f:
            f.write(html)
