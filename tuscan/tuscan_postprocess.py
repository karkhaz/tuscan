...
def load_and_process(path, patterns, results):
    ...
    results.append({"data": data, "file": basename(path)})
...
def do_postprocess(args):
    ...
    for toolchain in listdir(src_dir):
        ...
        for result in results:
            counter += 1
            with open(join(toolchain_dst, result["file"]), "w") as f:
                dump(result["data"], f)
            stderr.write("\r%5d/%5d" % (counter, total))
        stderr.write("\n")
